...
public class GUI {
    ...
    private JPanel colourGridHistory = new JPanel();
    ...
    private JButton helpButton = new JButton("Help & Info");
    ...
    //private Map<Integer, ArrayList<Integer>> multiValHistory = new HashMap<Integer, ArrayList<Integer>>();
    private LinkedList<ArrayList<Integer>> multiValHistory = new LinkedList<>();

    private JButton[] historyColours;
    ...
    //private JButton brushSliderButton = new JButton("Brush Size");
    ...
    private JLabel sliderLabel = new JLabel("         Adjust Brush Size", SwingConstants.CENTER);
    private int frameWidth, frameHeight;
    private ArrayList<LayerGroup> layerGroups;
    protected JColorChooser chooser = new JColorChooser();
    private JSlider brushSlider = new JSlider(JSlider.HORIZONTAL, 1, 10, 3);
    ...
    public GUI() {
        ...
        //filling the colour array with white colour values

        multiValHistory.add(new ArrayList<Integer>());
        multiValHistory.get(0).add(255);
        multiValHistory.get(0).add(255);
        multiValHistory.get(0).add(255);
        //multiValHistory.get(0).add(255);
        multiValHistory.add(new ArrayList<Integer>());
        multiValHistory.get(1).add(255);
        multiValHistory.get(1).add(255);
        multiValHistory.get(1).add(255);
        //multiValHistory.get(1).add(255);
        multiValHistory.add(new ArrayList<Integer>());
        multiValHistory.get(2).add(255);
        multiValHistory.get(2).add(255);
        multiValHistory.get(2).add(255);
        //multiValHistory.get(2).add(255);
        ...
        brushSlider.addChangeListener(changeListener);
        ...
        //brushSliderButton.addActionListener(actionListener);
        ...
        JPanel sliderPanelWraper = new JPanel(new FlowLayout());
        JPanel sliderPanel = new JPanel(new GridLayout(1, 2));
        sliderPanelWraper.add(sliderPanel);
        sliderPanel.add(sliderLabel);
        sliderPanel.add(brushSlider);
        toolBar.add(sliderPanelWraper);
        ...
        JPanel colourGridWrapper = new JPanel(new GridLayout(0, 1));
        colourGridWrapper.add(colourGrid);
        colourGrid.setLayout(new GridLayout(0, 3));
        colourGrid.setBorder(BorderFactory.createCompoundBorder(new EmptyBorder(0, 30, 0, 30), new LineBorder(Color.black, 1, true)));
        colourGridHistory.setLayout(new GridLayout(0, 3));
        colourGridHistory.setBorder(BorderFactory.createCompoundBorder(new EmptyBorder(10, 35, 10, 35), new LineBorder(Color.black, 1, true)));
        
        coloursPanel.add(colourLabPanel);
        coloursPanel.add(colourGridWrapper);
        coloursPanel.add(colourGridHistory);
        ...
        for (int i = 0; i < basicColours.length; i++) {
            ...
            basicColours[i].setMinimumSize(new Dimension(40, 20));
            basicColours[i].setMaximumSize(new Dimension(40, 20));
            basicColours[i].setPreferredSize(new Dimension(40, 20));
            colourGrid.add(basicColours[i]);
            basicColours[i].setContentAreaFilled(false);
            basicColours[i].setOpaque(true);
        }
        for (int i = 0; i < historyColours.length; i++) {
            historyColours[i] = new JButton();
            historyColours[i].addActionListener(actionListener);
            //historyColours[i].setBackground(new Color(basicColoursValues[i][0], basicColoursValues[i][1], basicColoursValues[i][2]));
            historyColours[i].setBackground(new Color(multiValHistory.get(i).get(0), multiValHistory.get(i).get(1), multiValHistory.get(i).get(2)));
            historyColours[i].setMinimumSize(new Dimension(40, 20));
            historyColours[i].setMaximumSize(new Dimension(40, 20));
            historyColours[i].setPreferredSize(new Dimension(40, 20));
            colourGridHistory.add(historyColours[i]);
            historyColours[i].setContentAreaFilled(false);
            historyColours[i].setOpaque(true);
        }

        mainFrame.setTitle("Pixel-based Graphics Editor");
    ...
    //implementing change listener for brush slider
    ChangeListener changeListener = new ChangeListener() {
        @Override
        public void stateChanged(ChangeEvent e) {
            JSlider source = (JSlider) e.getSource();
            toolManager.setBrushSize(source.getValue());
    ...
    ActionListener actionListener = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() == drawButton) {
                ...
                numLayersLabel.setText(""+layerManager.getLayerCount());
                ...
                toolManager.setTool(toolManager.currentTool.EYEDROP);
                    ...
                    class okListener implements ActionListener {
                            public void actionPerformed(ActionEvent e) {
                                ...
                                //converting int to Integer for use in dynamic list
                                Integer iRed = red;
                                Integer iGreen = green;
                                Integer iBlue = blue;
                                Integer iAlpha = alpha;
                                coloursPanel.setBackground(chosenColor);
                                layerManager.setColor(red, green, blue, alpha);
                                //Clearing current list and adding chosen colour values

                                multiValHistory.get(colIncrementer).clear();
                                multiValHistory.get(colIncrementer).add(iRed);
                                multiValHistory.get(colIncrementer).add(iGreen);
                                multiValHistory.get(colIncrementer).add(iBlue);
                                //setting background of the jButton array
                                historyColours[colIncrementer].setBackground(new Color(multiValHistory.get(colIncrementer).get(0), multiValHistory.get(colIncrementer).get(1), multiValHistory.get(colIncrementer).get(2)));
                                colIncrementer++;
                                //reset counter at button array limit
                                if (colIncrementer == 3){
                                    colIncrementer = 0;
                                }
            ...
            else if (e.getSource() == helpButton)
            {
                ...
                /*else if (e.getSource() == brushSliderButton){
                    JDialog sliderDialog = new JDialog(mainFrame, "Brush Size", true);
                    ...
                    for (int i = 0; i < layerGroups.size(); i++) {
                        ...
                        if (e.getSource() == interactedLayerGroup.selectLayer) {
                            ...
                            for(int x = 0; x < layerGroups.size(); x++)
                            {
                                layerGroups.get(x).panel.setBackground(Color.lightGray);
                            }
                    ...
                    for(int i = 0; i < historyColours.length; i++){
                        if(e.getSource() == historyColours[i]){
                            layerManager.setColor(multiValHistory.get(i).get(0),multiValHistory.get(i).get(1), multiValHistory.get(i).get(2),255);
                        }
                    }
                }
            }
